...
func (options UserSortOptions) Apply(query *gorm.DB) *gorm.DB {
	if columnName, ok := userSortColumns[options.SortBy]; ok {
...
func GetAllUsers(pageInfo *common.PageInfo, sortOptions ...UserSortOptions) (users []*User, total int64, err error) {
	...
	err = order.Apply(tx.Unscoped()).Limit(pageInfo.GetPageSize()).Offset(pageInfo.GetStartIdx()).Omit("password").Find(&users).Error
	if err != nil {
		tx.Rollback()
		return nil, 0, err
	}
...
func SearchUsers(keyword string, group string, role *int, status *int, startIdx int, num int, sortOptions ...UserSortOptions) ([]*User, int64, error) {
	...
	// 获取分页数据
	err = order.Apply(query).Omit("password").Limit(num).Offset(startIdx).Find(&users).Error
	if err != nil {
		tx.Rollback()
		return nil, 0, err
	}
...
func (user *User) Update(updatePassword bool) error {
	...
	if err = DB.Model(user).Omit("quota").Updates(newUser).Error; err != nil {
		return err
	}
...
func GetUserQuota(id int, fromDB bool) (quota int, err error) {
	database, err := walletUsesDatabase(id)
	if err != nil {
		return 0, err
	}
	if database {
...
func IncreaseUserQuota(id int, quota int, db bool) error {
	...
	return changeUserQuota(id, quota, db)
}
...
func changeUserQuota(id int, delta int, db bool) (err error) {
	if delta == 0 {
		return nil
	}
	batch, direct, err := beginWalletQuotaWrite(id, !db && common.BatchUpdateEnabled)
	if err != nil {
		return err
	}
	if !direct {
	...
	if batch {
		...
		return nil // the detached batch retains ownership until SQL success
	}
