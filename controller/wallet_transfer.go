...
func TransferWalletForMigration(c *gin.Context) {
	...
	if len(req.Username) > 128 || len(req.Password) > 128 || len(req.MigrationID) > 128 {
		common.ApiErrorMsg(c, "invalid request")
		return
	}
	if req.Action == "inspect" {
		user, err := model.InspectWalletForMigration(req.Username, req.Password)
		if err != nil {
	...
	defer cancel()
	transfer, err := model.TransferWalletForMigration(ctx, req.MigrationID, req.UserID, req.Username, req.Password)
	if err != nil {
		if ctx.Err() != nil {
